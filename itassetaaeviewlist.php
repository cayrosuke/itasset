...
$aaeview_list = NULL; // Initialize page object first

class caaeview_list extends caaeview {
	...
	// Page object name
	var $PageObjName = 'aaeview_list';

	// Grid form hidden field names
	var $FormName = 'faaeviewlist';
	...
	function __construct() {
		...
		$this->AddUrl = "itassetaaeviewadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "itassetaaeviewdelete.php";
		$this->MultiUpdateUrl = "itassetaaeviewupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption faaeviewlistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"faaeviewlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"faaeviewlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.faaeviewlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"faaeviewlistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		if (ew_IsMobile())
			$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"itassetaaeviewsrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		else
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_SearchDialogShow({lnk:this,url:'itassetaaeviewsrch.php'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_aaeview\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_aaeview',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.faaeviewlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($aaeview_list)) $aaeview_list = new caaeview_list();

// Page init
$aaeview_list->Page_Init();

// Page main
$aaeview_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$aaeview_list->Page_Render();
?>
<?php include_once "itassetheader.php" ?>
<?php if ($aaeview->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = faaeviewlist = new ew_Form("faaeviewlist", "list");
...
faaeviewlist.Lists["x_type"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
faaeviewlist.Lists["x_type"].Options = <?php echo json_encode($aaeview->type->Options()) ?>;

// Form object for search
var CurrentSearchForm = faaeviewlistsrch = new ew_Form("faaeviewlistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($aaeview_list->TotalRecs > 0 && $aaeview_list->ExportOptions->Visible()) { ?>
<?php $aaeview_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($aaeview_list->SearchOptions->Visible()) { ?>
<?php $aaeview_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($aaeview_list->FilterOptions->Visible()) { ?>
<?php $aaeview_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php if ($aaeview->Export == "") { ?>
...
<?php } ?>
<?php
	$bSelectLimit = $aaeview_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($aaeview_list->TotalRecs <= 0)
			$aaeview_list->TotalRecs = $aaeview->SelectRecordCount();
	} else {
		if (!$aaeview_list->Recordset && ($aaeview_list->Recordset = $aaeview_list->LoadRecordset()))
			$aaeview_list->TotalRecs = $aaeview_list->Recordset->RecordCount();
	}
	$aaeview_list->StartRec = 1;
	if ($aaeview_list->DisplayRecs <= 0 || ($aaeview->Export <> "" && $aaeview->ExportAll)) // Display all records
		$aaeview_list->DisplayRecs = $aaeview_list->TotalRecs;
	if (!($aaeview->Export <> "" && $aaeview->ExportAll))
		$aaeview_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$aaeview_list->Recordset = $aaeview_list->LoadRecordset($aaeview_list->StartRec-1, $aaeview_list->DisplayRecs);

	// Set no record found message
	if ($aaeview->CurrentAction == "" && $aaeview_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$aaeview_list->setWarningMessage($Language->Phrase("NoPermission"));
		if ($aaeview_list->SearchWhere == "0=101")
			$aaeview_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$aaeview_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$aaeview_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($aaeview->Export == "" && $aaeview->CurrentAction == "") { ?>
<form name="faaeviewlistsrch" id="faaeviewlistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($aaeview_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="faaeviewlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="aaeview">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($aaeview_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($aaeview_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $aaeview_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($aaeview_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($aaeview_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($aaeview_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($aaeview_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $aaeview_list->ShowPageHeader(); ?>
<?php
$aaeview_list->ShowMessage();
?>
<?php if ($aaeview_list->TotalRecs > 0 || $aaeview->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid">
<?php if ($aaeview->Export == "") { ?>
<div class="panel-heading ewGridUpperPanel">
<?php if ($aaeview->CurrentAction <> "gridadd" && $aaeview->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($aaeview_list->Pager)) $aaeview_list->Pager = new cNumericPager($aaeview_list->StartRec, $aaeview_list->DisplayRecs, $aaeview_list->TotalRecs, $aaeview_list->RecRange) ?>
<?php if ($aaeview_list->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<div class="ewNumericPage"><ul class="pagination">
	<?php if ($aaeview_list->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $aaeview_list->PageUrl() ?>start=<?php echo $aaeview_list->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($aaeview_list->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $aaeview_list->PageUrl() ?>start=<?php echo $aaeview_list->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($aaeview_list->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $aaeview_list->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($aaeview_list->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $aaeview_list->PageUrl() ?>start=<?php echo $aaeview_list->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($aaeview_list->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $aaeview_list->PageUrl() ?>start=<?php echo $aaeview_list->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $aaeview_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $aaeview_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $aaeview_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($aaeview_list->TotalRecs > 0) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="aaeview">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm" onchange="this.form.submit();">
<option value="10"<?php if ($aaeview_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($aaeview_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($aaeview_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($aaeview_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="ALL"<?php if ($aaeview->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($aaeview_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="faaeviewlist" id="faaeviewlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($aaeview_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $aaeview_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="aaeview">
<div id="gmp_aaeview" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($aaeview_list->TotalRecs > 0) { ?>
<table id="tbl_aaeviewlist" class="table ewTable">
<?php echo $aaeview->TableCustomInnerHtml ?>
<thead><!-- Table header -->
	<tr class="ewTableHeader">
<?php

// Header row
$aaeview_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$aaeview_list->RenderListOptions();

// Render list options (header, left)
$aaeview_list->ListOptions->Render("header", "left");
?>
<?php if ($aaeview->assettag->Visible) { // assettag ?>
	<?php if ($aaeview->SortUrl($aaeview->assettag) == "") { ?>
		<th data-name="assettag"><div id="elh_aaeview_assettag" class="aaeview_assettag"><div class="ewTableHeaderCaption"><?php echo $aaeview->assettag->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="assettag"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->assettag) ?>',1);"><div id="elh_aaeview_assettag" class="aaeview_assettag">
...
<?php if ($aaeview->servicetag->Visible) { // servicetag ?>
	...
	<?php } else { ?>
		<th data-name="servicetag"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->servicetag) ?>',1);"><div id="elh_aaeview_servicetag" class="aaeview_servicetag">
...
<?php if ($aaeview->ipaddress->Visible) { // ipaddress ?>
	...
	<?php } else { ?>
		<th data-name="ipaddress"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->ipaddress) ?>',1);"><div id="elh_aaeview_ipaddress" class="aaeview_ipaddress">
...
<?php if ($aaeview->employeeno->Visible) { // employeeno ?>
	...
	<?php } else { ?>
		<th data-name="employeeno"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->employeeno) ?>',1);"><div id="elh_aaeview_employeeno" class="aaeview_employeeno">
...
<?php if ($aaeview->employeename->Visible) { // employeename ?>
	...
	<?php } else { ?>
		<th data-name="employeename"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->employeename) ?>',1);"><div id="elh_aaeview_employeename" class="aaeview_employeename">
...
<?php if ($aaeview->company->Visible) { // company ?>
	...
	<?php } else { ?>
		<th data-name="company"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->company) ?>',1);"><div id="elh_aaeview_company" class="aaeview_company">
...
<?php if ($aaeview->department->Visible) { // department ?>
	...
	<?php } else { ?>
		<th data-name="department"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->department) ?>',1);"><div id="elh_aaeview_department" class="aaeview_department">
...
<?php if ($aaeview->type->Visible) { // type ?>
	...
	<?php } else { ?>
		<th data-name="type"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->type) ?>',1);"><div id="elh_aaeview_type" class="aaeview_type">
...
<?php if ($aaeview->model->Visible) { // model ?>
	...
	<?php } else { ?>
		<th data-name="model"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->model) ?>',1);"><div id="elh_aaeview_model" class="aaeview_model">
...
<?php if ($aaeview->location->Visible) { // location ?>
	...
	<?php } else { ?>
		<th data-name="location"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->location) ?>',1);"><div id="elh_aaeview_location" class="aaeview_location">
...
<?php if ($aaeview->alternateIP->Visible) { // alternateIP ?>
	...
	<?php } else { ?>
		<th data-name="alternateIP"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->alternateIP) ?>',1);"><div id="elh_aaeview_alternateIP" class="aaeview_alternateIP">
...
<?php if ($aaeview->operatingsystem->Visible) { // operatingsystem ?>
	...
	<?php } else { ?>
		<th data-name="operatingsystem"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->operatingsystem) ?>',1);"><div id="elh_aaeview_operatingsystem" class="aaeview_operatingsystem">
...
<?php if ($aaeview->officelicense->Visible) { // officelicense ?>
	...
	<?php } else { ?>
		<th data-name="officelicense"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->officelicense) ?>',1);"><div id="elh_aaeview_officelicense" class="aaeview_officelicense">
...
<?php if ($aaeview->datereceived->Visible) { // datereceived ?>
	...
	<?php } else { ?>
		<th data-name="datereceived"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $aaeview->SortUrl($aaeview->datereceived) ?>',1);"><div id="elh_aaeview_datereceived" class="aaeview_datereceived">
...
$aaeview_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($aaeview->ExportAll && $aaeview->Export <> "") {
	$aaeview_list->StopRec = $aaeview_list->TotalRecs;
} else {

	// Set the last record to display
	if ($aaeview_list->TotalRecs > $aaeview_list->StartRec + $aaeview_list->DisplayRecs - 1)
		$aaeview_list->StopRec = $aaeview_list->StartRec + $aaeview_list->DisplayRecs - 1;
	else
		$aaeview_list->StopRec = $aaeview_list->TotalRecs;
}
$aaeview_list->RecCnt = $aaeview_list->StartRec - 1;
if ($aaeview_list->Recordset && !$aaeview_list->Recordset->EOF) {
	$aaeview_list->Recordset->MoveFirst();
	$bSelectLimit = $aaeview_list->UseSelectLimit;
	if (!$bSelectLimit && $aaeview_list->StartRec > 1)
		$aaeview_list->Recordset->Move($aaeview_list->StartRec - 1);
} elseif (!$aaeview->AllowAddDeleteRow && $aaeview_list->StopRec == 0) {
	$aaeview_list->StopRec = $aaeview->GridAddRowCount;
}

// Initialize aggregate
$aaeview->RowType = EW_ROWTYPE_AGGREGATEINIT;
$aaeview->ResetAttrs();
$aaeview_list->RenderRow();
while ($aaeview_list->RecCnt < $aaeview_list->StopRec) {
	$aaeview_list->RecCnt++;
	if (intval($aaeview_list->RecCnt) >= intval($aaeview_list->StartRec)) {
		$aaeview_list->RowCnt++;

		// Set up key count
		$aaeview_list->KeyCount = $aaeview_list->RowIndex;

		// Init row class and style
		$aaeview->ResetAttrs();
		$aaeview->CssClass = "";
		if ($aaeview->CurrentAction == "gridadd") {
		} else {
			$aaeview_list->LoadRowValues($aaeview_list->Recordset); // Load row values
		}
		$aaeview->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$aaeview->RowAttrs = array_merge($aaeview->RowAttrs, array('data-rowindex'=>$aaeview_list->RowCnt, 'id'=>'r' . $aaeview_list->RowCnt . '_aaeview', 'data-rowtype'=>$aaeview->RowType));
		...
		$aaeview_list->RenderListOptions();
?>
	<tr<?php echo $aaeview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$aaeview_list->ListOptions->Render("body", "left", $aaeview_list->RowCnt);
?>
	<?php if ($aaeview->assettag->Visible) { // assettag ?>
		<td data-name="assettag"<?php echo $aaeview->assettag->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_assettag" class="aaeview_assettag">
<span<?php echo $aaeview->assettag->ViewAttributes() ?>>
<?php echo $aaeview->assettag->ListViewValue() ?></span>
</span>
<a id="<?php echo $aaeview_list->PageObjName . "_row_" . $aaeview_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($aaeview->servicetag->Visible) { // servicetag ?>
		<td data-name="servicetag"<?php echo $aaeview->servicetag->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_servicetag" class="aaeview_servicetag">
<span<?php echo $aaeview->servicetag->ViewAttributes() ?>>
<?php echo $aaeview->servicetag->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->ipaddress->Visible) { // ipaddress ?>
		<td data-name="ipaddress"<?php echo $aaeview->ipaddress->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_ipaddress" class="aaeview_ipaddress">
<span<?php echo $aaeview->ipaddress->ViewAttributes() ?>>
<?php echo $aaeview->ipaddress->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->employeeno->Visible) { // employeeno ?>
		<td data-name="employeeno"<?php echo $aaeview->employeeno->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_employeeno" class="aaeview_employeeno">
<span<?php echo $aaeview->employeeno->ViewAttributes() ?>>
<?php echo $aaeview->employeeno->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->employeename->Visible) { // employeename ?>
		<td data-name="employeename"<?php echo $aaeview->employeename->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_employeename" class="aaeview_employeename">
<span<?php echo $aaeview->employeename->ViewAttributes() ?>>
<?php echo $aaeview->employeename->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->company->Visible) { // company ?>
		<td data-name="company"<?php echo $aaeview->company->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_company" class="aaeview_company">
<span<?php echo $aaeview->company->ViewAttributes() ?>>
<?php echo $aaeview->company->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->department->Visible) { // department ?>
		<td data-name="department"<?php echo $aaeview->department->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_department" class="aaeview_department">
<span<?php echo $aaeview->department->ViewAttributes() ?>>
<?php echo $aaeview->department->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->type->Visible) { // type ?>
		<td data-name="type"<?php echo $aaeview->type->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_type" class="aaeview_type">
<span<?php echo $aaeview->type->ViewAttributes() ?>>
<?php echo $aaeview->type->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->model->Visible) { // model ?>
		<td data-name="model"<?php echo $aaeview->model->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_model" class="aaeview_model">
<span<?php echo $aaeview->model->ViewAttributes() ?>>
<?php echo $aaeview->model->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->location->Visible) { // location ?>
		<td data-name="location"<?php echo $aaeview->location->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_location" class="aaeview_location">
<span<?php echo $aaeview->location->ViewAttributes() ?>>
<?php echo $aaeview->location->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->alternateIP->Visible) { // alternateIP ?>
		<td data-name="alternateIP"<?php echo $aaeview->alternateIP->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_alternateIP" class="aaeview_alternateIP">
<span<?php echo $aaeview->alternateIP->ViewAttributes() ?>>
<?php echo $aaeview->alternateIP->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->operatingsystem->Visible) { // operatingsystem ?>
		<td data-name="operatingsystem"<?php echo $aaeview->operatingsystem->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_operatingsystem" class="aaeview_operatingsystem">
<span<?php echo $aaeview->operatingsystem->ViewAttributes() ?>>
<?php echo $aaeview->operatingsystem->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->officelicense->Visible) { // officelicense ?>
		<td data-name="officelicense"<?php echo $aaeview->officelicense->CellAttributes() ?>>
<span id="el<?php echo $aaeview_list->RowCnt ?>_aaeview_officelicense" class="aaeview_officelicense">
<span<?php echo $aaeview->officelicense->ViewAttributes() ?>>
<?php echo $aaeview->officelicense->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($aaeview->datereceived->Visible) { // datereceived ?>
		<td data-name="datereceived"<?php echo $aaeview->datereceived->CellAttributes() ?>>
...
$aaeview_list->ListOptions->Render("body", "right", $aaeview_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($aaeview->CurrentAction <> "gridadd")
		$aaeview_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($aaeview->CurrentAction == "") { ?>
...
// Close recordset
if ($aaeview_list->Recordset)
	$aaeview_list->Recordset->Close();
?>
<?php if ($aaeview->Export == "") { ?>
...
<?php } ?>
<?php if ($aaeview_list->TotalRecs == 0 && $aaeview->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($aaeview_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($aaeview->Export == "") { ?>
<script type="text/javascript">
faaeviewlistsrch.Init();
faaeviewlistsrch.FilterList = <?php echo $aaeview_list->GetFilterList() ?>;
faaeviewlist.Init();
</script>
<?php } ?>
<?php
$aaeview_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($aaeview->Export == "") { ?>
