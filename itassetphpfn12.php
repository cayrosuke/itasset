...
function ew_AutoLoad($class) {
	...
	if ($class == "GD") {
		...
		$file = "itassetewdbhelper12.php";
	} elseif (ew_StartsStr("c", $class)) {
		$file = str_replace("%cls%", substr($class, 1), "itasset%cls%info.php");
...
function CurrentProjectID() {
	...
	return "{6E5D033C-DA34-4C1B-AC0C-D8B1ECCFD39C}";
}
...
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "itassetewfile12.php?t=" . ew_Encrypt($fld->TblName, $key) ."&fn=" . ew_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "itassetewfile12.php?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($path . $val) .
